  ...
  implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect
{
  ...
  async handleConnection(client: SocketWithAuth) {
    ...
    const roomId = client.pollID;
    await client.join(roomId);

    const connectedClients = this.io.adapter.rooms?.get(roomId)?.size ?? 0;

    this.logger.debug(
      `userID: ${client.userID} joined room with name: ${roomId}`,
    );
    this.logger.debug(
      `Total clients connected to room '${roomId}': ${connectedClients}`,
    );
    ...
    this.io.to(roomId).emit('poll_updated', updatedPoll);
  }
  ...
  async handleDisconnect(client: SocketWithAuth) {
    ...
    this.logger.debug(`Number of connected sockets: ${sockets.size}`);
    this.logger.debug(
      `Total clients connected to room '${roomId}': ${clientCount}`,
    );

    // updatedPoll could be undefined if the the poll already started
    // in this case, the socket is disconnected, but no the poll state
    if (updatedPoll) {
